...
// how many sends actually succeeded, and per-attempt debug info so
// failures are diagnosable instead of just showing up as sentCount: 0.
async function sendToAll(subs, env, payload) {
  ...
  for (const sub of subs) {
    try {
      ...
      let bodySnippet = "";
      try { bodySnippet = (await res.clone().text()).slice(0, 200); } catch (e2) {}
      debug.push({ status: res.status, ok: res.ok, body: bodySnippet });
      // 404/410 = subscription no longer exists. 403 (BadJwtToken) means
      // this subscription was created against a different/older VAPID key
      // than the one currently configured — it will never succeed no
      // matter how many times we retry, so treat it the same as dead.
      const isDead = res.status === 404 || res.status === 410 || res.status === 403;
      if (!isDead) {
        stillValid.push(sub);
        if (res.ok) sentCount++;
      }
    } catch (e) {
      debug.push({ error: String(e && e.message ? e.message : e) });
      stillValid.push(sub);
    }
  }
  return { stillValid, sentCount, debug };
}
...
async function runCheck(env, force) {
  ...
  if (today === todayMonday && restrictedNow !== restrictedPrevWeek && (force || !boundaryAlreadySentToday)) {
    ...
    if (subs.length > 0) {
      const { stillValid, sentCount, debug } = await sendToAll(subs, env, payload);
      if (stillValid.length !== subs.length) { subs = stillValid; subsChanged = true; }
      result.boundaryNotice = { type: restrictedNow ? "restriction-started" : "restriction-lifted", sentCount, debug };
    } else {
  ...
  if (restrictedNow && (force || !dailyAlreadySentToday)) {
    ...
    if (!loggedToday) {
      if (subs.length > 0) {
        ...
        const { stillValid, sentCount, debug } = await sendToAll(subs, env, payload);
        if (stillValid.length !== subs.length) { subs = stillValid; subsChanged = true; }
        result.dailyReminder = { sent: true, sentCount, debug };
      } else {
...
export default {
  async scheduled(controller, env, ctx) {
    // Never let an uncaught error reach the platform — a rejected
    // promise here causes Cloudflare to treat the whole invocation as
    // failed and automatically retry it repeatedly (which is very
    // likely what caused the burst of invocations without ever
    // successfully sending). Catch everything, log it, and store the
    // last error so it's inspectable later via the URL below.
    try {
      await runCheck(env, false);
      await env.HOURS_KV.put(`last-error:${SYNC_USER}`, ""); // clear on success
    } catch (e) {
      const msg = `${new Date().toISOString()} — ${e && e.stack ? e.stack : String(e)}`;
      console.error("scheduled run failed:", msg);
      await env.HOURS_KV.put(`last-error:${SYNC_USER}`, msg).catch(() => {});
    }
  },
  ...
  // Add ?lasterror=1 to just read back the last error a scheduled run
  // hit, without running anything.
  async fetch(request, env, ctx) {
    const url = new URL(request.url);

    if (url.searchParams.get("lasterror") === "1") {
      const lastError = (await env.HOURS_KV.get(`last-error:${SYNC_USER}`)) || "(none recorded)";
      return new Response(JSON.stringify({ lastScheduledError: lastError }, null, 2), { headers: { "content-type": "application/json" } });
    }

    const force = url.searchParams.get("force") === "1";
    let result;
    try {
      result = await runCheck(env, force);
    } catch (e) {
      const msg = e && e.stack ? e.stack : String(e);
      return new Response(JSON.stringify({ ran: false, error: msg }, null, 2), { status: 500, headers: { "content-type": "application/json" } });
    }
